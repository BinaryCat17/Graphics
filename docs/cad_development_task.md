...
Ниже формулируется объёмная стартовая задача для внедрения декларативных 3D-моделей/сборок, расчётов прочности и кинематики, а также Vulkan-рендеринга без внешних зависимостей на парсинг YAML и STEP. Детализированная схема формата YAML и примерные сцены вынесены в `docs/cad_yaml_spec.md` и каталог `assets/scenes/`.
...
- Не использовать сторонние парсеры YAML/STEP; допускаются лишь стандартные заголовки C и уже присутствующие зависимости проекта.
