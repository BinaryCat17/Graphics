...
void vk_run_compute_graph_image(VulkanRendererState* state, const char* glsl_source, int width, int height) {
    if (!state || !glsl_source) return;
    
    vk_ensure_compute_target(state, width, height);
    if (!state->compute_target_view) return;
    ...
    // 2. Create Layout (Set 0 Binding 0: Storage Image)
    VkDescriptorSetLayoutBinding binding = { 
        .binding = 0, 
    ...
    // Push Constants (Time, W, H) - 12 bytes
    ...
    // Transition to General (if not already - simpler to enforce here)
    // Actually, ensure_compute_target puts it in GENERAL. 
    // And Sampling uses GENERAL. 
    // So we are good.
    
    vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline);
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, pipe_layout, 0, 1, &set, 0, NULL);
    
    struct { float t; float w; float h; } pc = { (float)platform_get_time_ms() / 1000.0f, (float)width, (float)height };
    ...
    vkQueueSubmit(state->queue, 1, &submit, fence);
    vkWaitForFences(state->device, 1, &fence, VK_TRUE, 1000000000); 
